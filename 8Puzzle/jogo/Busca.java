...
public abstract class Busca {
	...
	protected Status status = new Status(); // a classe que tem o status
	protected MostraStatusConsole mstatus = null; // a classe que mostra o
													// stauts

	private Map<Estado, Integer> fechados = null; // mapeia o estado para um
													// custo g
	...
	public List<Nodo> sucessores(Nodo pai) {
		return soNovos(pai.estado.sucessores(), pai); // lista de todos os
														// sucessores
	}
	...
	public List<Nodo> antecessores(Nodo pai) {
		try {
			...
			System.err.println("O estado " + pai.estado
					+ " nao implementa antecessores!");
			return new LinkedList<Nodo>();
		}
	}

	private List<Nodo> soNovos(List<Estado> estados, Nodo pai) {
		List<Nodo> sucNodo = new LinkedList<Nodo>(); // a lista de sucessores
														// novos
		for (Estado e : estados) {
			...
			if (podar) {
				if (usarFechado && fechados != null) {
					...
					if (custo == null || filho.g < custo.intValue()) { // nao
																		// esta
																		// em
																		// fechados
																		// ou
				...
				} else if (filho.ehDescendenteNovo(pai)) { // poda os filhos que
															// tem um ascensor
															// igual a ele
					sucNodo.add(filho);
				}
