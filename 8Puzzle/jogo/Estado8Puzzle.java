...
import jogo.Antecessor;
import jogo.BuscaLargura;
import jogo.Estado;
import jogo.Heuristica;
import jogo.MostraStatusConsole;
import jogo.Nodo;
...
public class Estado8Puzzle implements Estado, Heuristica, Antecessor {
		...
		for (int l = 0; l < tam; l++) {
			for (int c = 0; c < tam; c++) {
				...
				if (t.tabuleiro[l][c] != meta.tabuleiro[l][c]) {
					...
					troca(t.tabuleiro, l, c, t.getLinNro(vlOk),
							t.getColNro(vlOk));
					nroTrocas++;
				}
			}
	...
	static public Estado8Puzzle getEstadoFacil() {
		Estado8Puzzle e8 = new Estado8Puzzle(new int[][] { { 8, 1, 3 },
				{ 0, 7, 2 }, { 6, 5, 4 } });
		e8.setPosBranco();
		return e8;
	}
	...
	static public Estado8Puzzle getEstadoDificil() {
		Estado8Puzzle e8 = new Estado8Puzzle(new int[][] { { 7, 8, 6 },
				{ 2, 3, 5 }, { 1, 4, 0 } });
		e8.setPosBranco();
		return e8;
	}
	...
	static public Estado8Puzzle getEstadoMuitoDificil() {
		...
		Estado8Puzzle e8 = new Estado8Puzzle(new int[][] { { 2, 3, 4 },
				{ 1, 0, 5 }, { 8, 7, 6 } });
		e8.setPosBranco();
		return e8;
	}
	...
	private static Estado8Puzzle setEstadoMeta() {
		Estado8Puzzle e8 = new Estado8Puzzle(new int[][] { { 1, 2, 3 },
				{ 8, 0, 4 }, { 7, 6, 5 } });
		e8.setPosBranco();
		return e8;
	}
	...
	public static void main(String[] a) {
		...
		System.out.println("estado inicial (h=" + ((Heuristica) e8).h() + ") ="
				+ e8);
		...
		if (s != null) {
			System.out.println("solucao (" + s.getProfundidade() + ")= "
					+ s.montaCaminho());
		}
	}
